...
app.post('/image-generation', async (req, res) => {
    try {
        ...
        if (containsNSFW(prompt)) {
            ...
            const trackingid = grc();
            admin.storage().bucket().upload(imageBytes , {
                destination:`path/in/firebase/storage/${trackingid}/image.jpg`,
                metadata:{
                    contentType:"image/jpeg"
                }
            }).then(()=>{
                admin.firestore().collection('image-generation').add({
                    user:uid,
                    prompt:prompt,
